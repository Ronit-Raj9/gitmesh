...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const saveSessionsToStorage = useCallback(() => {
    if (typeof window !== 'undefined') {
      try {
        ...
        localStorage.setItem('beetle_chat_sessions', JSON.stringify(sessionsToSave));
        localStorage.setItem('beetle_chat_active_session', state.activeSessionId || '');
      } catch (error) {
  ...
  const loadSessionsFromStorage = useCallback(() => {
    if (typeof window !== 'undefined') {
      try {
        const savedSessions = localStorage.getItem('beetle_chat_sessions');
        const activeSessionId = localStorage.getItem('beetle_chat_active_session');
